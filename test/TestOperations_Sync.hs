...
test_syncSuccessful =
  TestLabel "sync successful" $
    TestCase $ do
      ...
      assertEqual "sync successful" (Right ()) (fst operationResult)
...
test_sync_RemoteFailed_LoopSetup =
  TestLabel "remote loop-setup failure handled" $
    TestCase $ do
      ...
      let mockExecResults = [D.loopSetupExec False D.remoteOp]
      ...
      assertEqual "sync failed" expectedError (fst operationResult)
